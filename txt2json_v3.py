...
t2j_dict = {'Q': 'question','C': 'corpus','A': 'answer'}
...
class txt2json:
    ...
    def deal_txt(self, start=None, stop=None):
        ...
        for q_number in range(self.max_q):
            try:
                self.load_txt('CQA/'+str(q_number+1)+'.txt')
                if len(self.context)==10:
                    self._assign2json()
                elif len(self.context)!=10:
                    print("CQA error corpus :", str(q_number+1))
                    self.errCount+=1
                self.save('JsonResult/'+str(q_number+1)+'.json')
            except FileNotFoundError:
                pass
                #print('Check your path, I cant find your file {0} '.format(q_number+1))    
            self.reset()
