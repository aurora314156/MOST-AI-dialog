                    ...
                    if flag == 1:
                        ...
                        if tempS != "":
                            ...
                            for c in cutRes:
                                tempL.append(c)
                            aList.append(tempL)
                        # if answer is wrong format, then output split ans to json
                        # and write this question number to txt
                        if len(aList) == 4:
                            outputFile(cList, qList, aList, ind)
                        else:
                            # force answer split into four selection
                            aList, tempL = [], []
                            tempA = ""
                            f, count = 1 , 0
                            flag = int((len(s)/4))
                            for i in range(len(s)):
                                tempA += s[i]
                                count += 1
                                if count == (flag*f) and f !=4:
                                    tempL = []
                                    cutRes = jiebaCut(tempA)
                                    for c in cutRes:
                                        tempL.append(c)
                                    f += 1
                                    aList.append(tempL)
                            ...
                            outputFile(cList, qList, aList, ind)
                            wrongind += 1
                            with open("SplitAndNum.txt", 'a') as file:
                                file.write(str(f))
                                file.write("\n")
