...
class txt2json:
    def __init__(self, min_question=0, max_question=10000):
    ...
    def deal_txt(self, start=None, stop=None):
        ...
        for q_number in range(self.min_q, self.max_q):
            try:
                self.load_txt('test/'+str(q_number)+'.txt')
                self._assign2json()
                self.save('test_result/'+str(q_number)+'.json')
...
#         print('output : '+os.path.join(path, file_name))
    ...
    def _assign2json(self):
        ...
        if len(self.json_context['answer']) != ans_line_num:
            self.init_json()
    ...
    def _assign_txt(self, step):
        ...
        ins = self.get_ins()
        if ins is None:
            raise ValueError('check your file have C Q A.')
